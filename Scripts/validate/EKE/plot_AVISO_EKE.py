...
AVISO_EKE=np.zeros(shape=(165,161))
AVISO_lon=np.zeros(shape=(165,161))
AVISO_lat=np.zeros(shape=(165,161))
...
# Load AVISO EKE data from file
#-------------------------------------------------------------------------------
file=input_dir+'ROMS_eke.txt'
REKE = np.loadtxt(file,unpack=True)
ROMS_EKE=np.zeros(shape=hmask.shape)

count=0
for i in range(0,402):
  for j in range(0,482):
    ROMS_EKE[j,i]=REKE[count]
    count=count+1
...
plt.clf()
m=Basemap(projection='merc',llcrnrlat=lat[0,0],urcrnrlat=lat[481-25,401],\
          llcrnrlon=lon[0,0],urcrnrlon=lon[481-25,401],lat_ts=20,resolution='f')
...
xi, yi = m(lon2,lat2)
cs = m.pcolormesh(xi,yi,np.log10(AVISO_EKE),cmap=cm.gist_ncar,vmin=-5, vmax = -3)
...
# Save figure to file
plot_file=plot_dir+'AVISO_EKE.png'
