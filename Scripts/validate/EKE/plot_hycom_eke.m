...
eval(['load ',matdir,'eke_JUNE_hycom.mat']);
eval(['load ',matdir,'hycom_JUNE_meanvls.mat']);

for i=1:36
  [uu,vv]=regridromsvels2d(squeeze(meanu(i,:,:))',...
                           squeeze(meanv(i,:,:))');
  ...
  for j=1:size(h,2)
    ...
    if (z100ind(i,j)==37)
      ...
      eke100(i,j)=sum(squeeze(squeeze( ke(z100ind(i,j)+1:end,i,j))).*...
       ...
       ke100(i,j)=sum(squeeze(squeeze(mke(z100ind(i,j)+1:end,i,j))).*...
                      squeeze(squeeze(abs(h_layer(i,j,z100ind(i,j)+1:end)))))./...
             squeeze(squeeze(abs(h_layer(i,j,end)-h_layer(i,j,z100ind(i,j)))));
    end
    ...
    if (z50ind(i,j)==37)
      eke50(i,j)=squeeze(squeeze( ke(end,i,j)));
       ke50(i,j)=squeeze(squeeze(mke(end,i,j)));
    else
      eke50(i,j)=sum(squeeze(squeeze( ke(z50ind(i,j)+1:end,i,j))).*...
       ...
       ke50(i,j)=sum(squeeze(squeeze(mke(z50ind(i,j)+1:end,i,j))).*...
                     squeeze(squeeze(abs(h_layer(i,j,z50ind(i,j)+1:end)))))./...
          squeeze(squeeze(abs(h_layer(i,j,end)-h_layer(i,j,z50ind(i,j)))));
    end   
  end
...
plot_procedure(lon,lat,mask,log(eke50).*h50mask./h50mask,[5.5 10],...
                 ['HYCOM'],['Upper 50m EKE']);
eval(['export_fig ',plotdir,'EKE50_JUNE_hycom.png -png -r150 -painters']);

plot_procedure(lon,lat,mask,log( ke50).*h50mask./h50mask,[5.5 10],...
                 ['HYCOM'],['Upper 50m KE']);
eval(['export_fig ',plotdir,'KE50_JUNE_hycom.png -png -r150 -painters']);

plot_procedure(lon,lat,mask,log(eke100).*h100mask./h100mask,[5.5 10],...
                ['HYCOM'],['Upper 100m EKE']);
eval(['export_fig ',plotdir,'EKE100_JUNE_hycom.png -png -r150 -painters']);

plot_procedure(lon,lat,mask,log( ke100).*h100mask./h100mask,[5.5 10],...
                ['HYCOM'],['Upper 100m KE']);
eval(['export_fig ',plotdir,'KE100_JUNE_hycom.png -png -r150 -painters']);
...
%=========================================================================%
eval(['load ',matdir,'mean_vels.mat']);
...
for i=6:6
  ...
  plot_procedure(lon,lat,mask,log(ke),[4.5 7.5],['Exp ',sprintf('%02i',i)],...
                                          [labs{i},' Mean KE']);
  eval(['export_fig ',plotdir,'meanKE_exp',sprintf('%01i',i),'.png ',...
       '-png -r150 -painters']);
  ...
  plot_procedure(lon,lat,mask,log(eke),[4.5 7.5],['TNU=40'],...%['Exp ',sprintf('%02i',i)],...
                                           [labs{i},' Eddy KE']);
  eval(['export_fig ',plotdir,'EKE_exp',sprintf('%01i',i),'.png ',...
       '-png -r150 -painters']);
