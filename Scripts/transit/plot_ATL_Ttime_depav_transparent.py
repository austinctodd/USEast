#===============================================================================
#
# SCRIPT :  plot_ATL_rtime_depav_transparent.py
#
# PURPOSE : Ingest input data from US East water age model and plot the depth-
#           averaged residence time on a map of just the US Atlantic Coast.
#           If month 1, plot with US East Coast basemap. Otherwise, plot with
#           transparent background and no coastline.
#
# METHOD :
#
...
fwd_file   = '/Volumes/Black_box/Data/USeast-age/output/clim/averages/'+\
             'avg_3hrly.nc'
plot_dir   = '/Volumes/Black_box/Data/PLOTS/USeast-age/rtime/ATL/depav/stacked/'

#-------------------------------------------------------------------------------
# Print message to user
#-------------------------------------------------------------------------------
print "----------------------------------------------------------------------"
print " RUNNING: Script plot_ATL_rtime_depav.py"
print "----------------------------------------------------------------------"
print ""
...
for i in range(0,12):
  ...
  rtime=np.nanmean(input_data.variables['age_02'][stind:enind,:,:,:401-24],axis=0)

  #------------------------------------------------------------------------
  # Load in temp,salt, age, residence time
  #------------------------------------------------------------------------
  zeta  =np.nanmean(fwd_data.variables['zeta'  ][stind:enind,  :,:401-24],axis=0)
  ...
  if i==0:
    ...
    cn = m.contour(   xi,yi,rtime2,[365.25,547.875,730.5],colors='0.5',linewidth=1.0)
    ...
    m.contour(   xi,yi,rtime2,[365.25,547.875,730.5],colors='0.5',linewidth=1.0)
    cm.gist_ncar.set_bad('blue',alpha=0.0)
