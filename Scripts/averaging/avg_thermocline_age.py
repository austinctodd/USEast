...
output_data.createDimension('s_rho'     ,2) # two layers - above & below pcline
...
for tdim in range(0,1):#len(input_data.dimensions['ocean_time']-1)):
  ...
  for i in range(0,M):
    for j in range(0,L):
      ...
      #--------------------------------------------------------------------
      mld[tdim,i,j] = abs(z[drdZ_ind,i,j])
      age_01[tdim,0,i,j]=sum(  Hz[0:drdZ_ind-1 ,i,j]*\
                             age1[0:drdZ_ind-1 ,i,j])/abs(z[drdZ_ind-1,i,j])
      age_01[tdim,1,i,j]=sum(  Hz[  drdZ_ind:35,i,j]*\
                             age1[  drdZ_ind:35,i,j])/abs(z[drdZ_ind  ,i,j])
      age_02[tdim,0,i,j]=sum(  Hz[0:drdZ_ind-1 ,i,j]*\
                             age2[0:drdZ_ind-1 ,i,j])/abs(z[drdZ_ind-1,i,j])
      age_02[tdim,1,i,j]=sum(  Hz[  drdZ_ind:35,i,j]*\
                             age2[  drdZ_ind:35,i,j])/abs(z[drdZ_ind  ,i,j])
      age_03[tdim,0,i,j]=sum(  Hz[0:drdZ_ind-1 ,i,j]*\
                             age3[0:drdZ_ind-1 ,i,j])/abs(z[drdZ_ind-1,i,j])
      age_03[tdim,1,i,j]=sum(  Hz[  drdZ_ind:35,i,j]*\
