  ...
  for i in range(0,M):
    for j in range(0,L):
      ...
      if mask[i,j]>0:
        ...
        age_01[tdim,i,j]=sum(np.squeeze(   Hz[drdZ_ind::,i,j])*\
                                         age1[drdZ_ind::])/\
                             np.squeeze(abs(z[drdZ_ind,i,j]))
        age_02[tdim,i,j]=sum(np.squeeze(   Hz[drdZ_ind::,i,j])*\
                                         age2[drdZ_ind::])/\
                             np.squeeze(abs(z[drdZ_ind,i,j]))
        age_03[tdim,i,j]=sum(np.squeeze(   Hz[drdZ_ind::,i,j])*\
                                         age3[drdZ_ind::])/\
                             np.squeeze(abs(z[drdZ_ind,i,j]))

        # Cell is within GOM shelf scope
        if Gscope[i,j]>0:
          rtime_1[tdim,i,j]=sum(np.squeeze(    Hz[drdZ_ind::,i,j])*\
                                           Grtime[drdZ_ind::])/\
                                np.squeeze(abs( z[drdZ_ind,i,j]))
        else:
          rtime_1[tdim,i,j]=np.nan
        
        # Cell is within shelf scope
        if Gscope[i,j]>0:
          rtime_2[tdim,i,j]=sum(np.squeeze(   Hz[drdZ_ind::,i,j])*\
                                           rtime[drdZ_ind::])/\
                                np.squeeze(abs(z[drdZ_ind,i,j]))
        else:
          rtime_2[tdim,i,j]=np.nan
    
      # Cell is over land
      else:
        mld[    tdim,i,j] = np.nan
        age_01[ tdim,i,j] = np.nan
        age_02[ tdim,i,j] = np.nan
        age_03[ tdim,i,j] = np.nan
        rtime_1[tdim,i,j] = np.nan
        rtime_2[tdim,i,j] = np.nan
