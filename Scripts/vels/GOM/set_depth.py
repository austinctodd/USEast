...
if (nargin < 8):
  print ' '
  print '*** Error:  SET_DEPTH - too few arguments.'
  print '                     number of supplied arguments: '+\
        num2str(nargin);
  print '                     number of required arguments: 8'
  print ' '
#endif

if (Vtransform < 1 || Vtransform > 2):
  print ' '
  print '*** Error:  SET_DEPTH - Illegal parameter Vtransform = '+\
        num2str(Vtransfrom)
  print ' '
#endif
...
if (Vstretching < 1 || Vstretching > 4):
  print ' '
  print '*** Error:  SET_DEPTH - Illegal parameter Vstretching = '+\
        num2str(Vstretching)
  print ' '
#endif
...
if (hc > min(min(h)) && Vtransform == 1):
  print ' '
  print '*** Error:  SET_DEPTH - critical depth exceeds minimum'+\
        ' bathymetry value.'
        print '                        Vtransform = '+num2str(Vtransform)
        print '                        hc         = '+num2str(hc)
        print '                        hmax       = '+num2str(min(min(h)))
        print ' '
        return
#endif
...
if (report),:
  print ' '
  if (Vtransform == 1):
    print 'Vtransform  = '+num2str(Vtransform)+'   original ROMS'
  else if (Vtransform == 2)
    print 'Vtransform  = '+num2str(Vtransform)+'   ROMS-UCLA'
  #end Vtransform if
  
  switch ( igrid ),
    case 1
      print ['   igrid    = ',num2str(igrid),                            ...
