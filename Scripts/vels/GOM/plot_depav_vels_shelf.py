...
fwd_file   = '/Volumes/Black_box/Data/USeast-age/output/clim/averages/'+\
              'avg_3hrly.nc'
plot_dir   = '/Volumes/Black_box/Data/PLOTS/USeast-age/vels/GOM/depth-averaged/shelf/'
...
for i in range(0,12):
  ...
  u2   =np.ma.array(u2   ,mask=scope[1:481,1:401])
  v2   =np.ma.array(v2   ,mask=scope[1:481,1:401])
  umag2=np.ma.array(umag2,mask=scope[1:481,1:401])
  ...
  # Plot Data
  xi, yi = m(lon2,lat2)
  cs = m.pcolormesh(xi,yi,umag2,cmap=cm.gist_ncar_r,vmin=0.005, vmax = 0.25)
  xi, yi = m(lon,lat)
  cM = m.contour(   xi,yi,scope2,[1,1],colors='k',linewidth=2.0)
  
  # transform vectors to projection grid.
  uproj,vproj,xx,yy = m.transform_vector(u2[::2,::2]*5,v2[::2,::2]*5,\
                                        lonu[0,::2].squeeze(),latu[::2,0].squeeze(),\
  ...
  # Add Colorbar
  clabels=np.array([0.005,0.05,0.1,0.15,0.2,0.25])#,2.75,3])
  cbar = m.colorbar(cs, location='bottom', pad="4%",ticks=clabels,extend='max')
  cbar.ax.set_xticklabels([0.5,5,10,15,20,25])#,2.75,3])
